using Microsoft.Data.SqlClient;
using ClickAndGoApp.Models;

namespace ClickAndGoApp.DAL;

public class StoreDAL : IStoreDAL
{
    private readonly DBConnection _db;

    public StoreDAL(DBConnection db)
    {
        _db = db;
    }

    public async Task<Store> GetStoreAsync(int storeId)
    {
        using SqlConnection conn = _db.GetConnexion();
        await conn.OpenAsync();

        string query = @"
            SELECT storeId, name, address
            FROM Store
            WHERE storeId = @storeId";

        using SqlCommand cmd = new SqlCommand(query, conn);
        cmd.Parameters.AddWithValue("@storeId", storeId);

        using SqlDataReader reader = await cmd.ExecuteReaderAsync();

        if (await reader.ReadAsync())
        {
            return new Store(
                (int)reader["storeId"],
                (string)reader["name"],
                (string)reader["address"]
            );
        }
        return null;
    }

    public Task<List<Store>> GetAllStoresAsync() => Task.FromResult<List<Store>>(null);
    public Task<List<TimeSlot>> GetAvailableTimeSlotsAsync(int storeId) => Task.FromResult<List<TimeSlot>>(null);
}
